...
Table*
CreateTable(int philosophersCount, int minDurationEatMs, int maxDurationEatMs,
...
int Eat(Table* pTable, Philosopher* pPhilosopher, struct timespec tw, int i)
{
    ...
    if (pPhilosopher->IsEating == true)
    {
        ...
        //struct timespec tw1 = RandomTimeMs(1, 2);
        //nanosleep(&tw1, NULL);
        return 1;
    }
    //pthread_mutex_unlock(pTable->pMutex);
...
void DoEatAll(Table* pTable)
{
    ...
    for (int i = 0; i < 20000000; i++)
    {
        ...
        struct timespec tw = RandomTimeMs(5, 15);
        ...
        struct timespec twb = RandomTimeMs(0, 2);
        //LogTableInfo(pTable);
...
void DoEatAll1(Table* pTable)
{
    ...
    for (int i = 0; i < 20000000; i++)
    {
        struct timespec twb = RandomTimeMs(10, 10);
