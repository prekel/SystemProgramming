...
/// Если f не равно a то функция, в которой используется макрос вернёт
/// UNSUCCESSFUL (-1).
///
/// \param f Проверяемое целое число или выражение, возвращающее целое число.
/// \param a Число, которому должно быть равно f.
#define RETURN_UNSUCCESSFUL_IF_NOT_EQUAL(f, a) do { \
    int tmp = (f); \
    if (tmp != a) \
    { \
        return UNSUCCESSFUL; \
